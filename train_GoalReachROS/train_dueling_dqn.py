...
def train_dueling_dqn():
    cfg = Config(algo='Dueling', env_name=ENV_NAME)
    try:
    ...
    log_dir = os.path.join("results_GoalReachROS/DuelingDQN", f"DuelingDQN_{curr_time}")
    ...
    with tqdm(total=cfg.episodes, desc=f"DuelingDQN {ENV_NAME}", unit="ep", dynamic_ncols=True, colour='yellow', position=tqdm_position, leave=True) as pbar:      
    ...
    plt.plot(return_list,  color="#023a85", label="Episode Return")
    ...
    plt.title(f'Dueling DQN Training Result on {ENV_NAME}')
    plt.xlabel('Episodes')
    plt.ylabel('Return')
    plt.legend()
