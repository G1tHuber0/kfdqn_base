import datetime
import math
import os
import sys
import time
from pathlib import Path
from collections import deque  # 用于滑动窗口统计成功率

import gymnasium as gym
import matplotlib.pyplot as plt
from torch.utils.tensorboard import SummaryWriter
from tqdm import tqdm
import csv
import numpy as np

# 设置项目根目录
ROOT_DIR = Path(__file__).resolve().parents[1]
if str(ROOT_DIR) not in sys.path:
    sys.path.insert(0, str(ROOT_DIR))

import envs_ros  # noqa: F401
from config import Config
from utils.replay_buffer import ReplayBuffer
from agents.dqn_agent import DQNAgent
from utils.run_artifacts import save_run_config, save_metrics
from utils.metrics import compute_training_metrics
from utils.seeding import episode_seed, seed_everything

# --- 常量与超参数配置 ---
ENV_NAME = "GoalReachROS-v0"
SUCCESS_WINDOW_SIZE = 50  # 统计最近10个回合的平均成功率
SUCCESS_THRESHOLD = -110.0
SUCCESS_REWARD_CUTOFF = -200.0
SUCCESS_TARGET_COUNT = 250
REWARD_BIN_RULES = [
    {"label": "Fail (-200)", "left": -200.0, "right": -200.0, "left_open": False, "right_open": False},
    {"label": "Success (Normal)", "left": -200.0, "right": -100.0, "left_open": True, "right_open": False},
    {"label": "Success (High Performance)", "left": -100.0, "right": math.inf, "left_open": True, "right_open": False},
]
REWARD_NORM_RANGE = (-200.0, 0.0)
CUMULATIVE_TARGET = math.inf

SILENT = os.environ.get("TRAIN_SILENT", "0") == "1"

def log_line(msg: str):
    if not SILENT:
        print(msg)

def log_tqdm(msg: str):
    if not SILENT:
        tqdm.write(msg)

def make_env(env_name: str = ENV_NAME):
    # 确保传入 sync_mode 等参数（取决于你的 Config 是否包含）
    env = gym.make(env_name)
    return env

def train_dqn():
    # 1. 加载配置
    cfg = Config(algo='DQN', env_name=ENV_NAME)
    try:
        cfg.episodes = int(os.environ.get("TRAIN_EP", cfg.episodes))
    except Exception:
        pass

    # 2. 创建实验目录
    curr_time = datetime.datetime.now().strftime("%Y%m%d_%H%M")
    log_dir = os.path.join("results_GoalReachROS/DQN", f"DQN_{curr_time}")
    if not os.path.exists(log_dir):
        os.makedirs(log_dir)
    
    save_run_config(log_dir, cfg)
    writer = SummaryWriter(log_dir=log_dir)
    
    log_line(f"\n{'='*60}")
    log_line(f"开始训练 DQN | 环境: {cfg.env_name} | 设备: {cfg.device}")
    log_line(f"成功率滑动窗口: {SUCCESS_WINDOW_SIZE} 回合")
    log_line(f"TensorBoard: {log_dir}")
    log_line(f"{'='*60}\n")
    
    # 3. 初始化环境与智能体
    base_seed = cfg.seed
    env = make_env(cfg.env_name)
    seed_everything(base_seed, env=env)
    
    agent = DQNAgent(cfg)
    buffer = ReplayBuffer(cfg.buffer_size)
    
    # 4. 初始化统计变量
    return_list = []
    total_steps = 0  # 全局总步数
    
    # 使用 deque 存储最近 N 回合的成功情况 (1=成功, 0=失败)
    success_sliding_window = deque(maxlen=SUCCESS_WINDOW_SIZE)

    # 5. 主训练循环
    tqdm_position = int(os.environ.get("TQDM_POSITION", "0"))
    
    # 使用 tqdm 创建进度条，desc 显示环境名
    with tqdm(total=cfg.episodes, desc=f"DQN {ENV_NAME}", unit="ep", 
              dynamic_ncols=True, colour='green', position=tqdm_position, leave=True) as pbar:
        
        for i in range(cfg.episodes):
            agent.update_epsilon(i)
            state, _ = env.reset(seed=episode_seed(base_seed, i))
            
            done = False
            episode_return = 0
            episode_loss = 0  
            update_count = 0  
            episode_steps = 0 
            last_info = {}
            
            # --- 回合内循环 ---
            while not done:
                # 动作选择与环境交互
                action = agent.take_action(state)
                next_state, reward, terminated, truncated, info = env.step(action)
                done = terminated or truncated
                
                # 存入经验池
                buffer.add(state, action, reward, next_state, done)
                state = next_state
                
                # 累加指标
                episode_return += reward
                total_steps += 1
                episode_steps += 1
                last_info = info
                
                # 网络更新
                if buffer.size() > cfg.minimal_size:
                    b_s, b_a, b_r, b_ns, b_d = buffer.sample(cfg.batch_size)
                    transition_dict = {
                        'states': b_s, 'actions': b_a, 'next_states': b_ns, 
                        'rewards': b_r, 'dones': b_d
                    }
                    loss = agent.update(transition_dict) 
                    if loss is not None:
                        episode_loss += loss
                        update_count += 1
                
                # [优化] 实时更新进度条后缀：显示全局总步数和当前回合步数
                pbar.set_postfix({
                    'TotalSteps': total_steps,
                    'EpStep': f"{episode_steps}/{env.unwrapped.max_steps if hasattr(env.unwrapped, 'max_steps') else '?'}",
                })

            # --- 回合结束后的统计 ---
            # 记录成功情况
            success_val = 1 if last_info.get("is_success") else 0
            success_sliding_window.append(success_val)
            
            # 计算滑动窗口成功率
            rolling_success_rate = sum(success_sliding_window) / len(success_sliding_window)
            return_list.append(episode_return)
            
            # 计算平均损失
            avg_loss = episode_loss / update_count if update_count > 0 else 0
            
            # --- TensorBoard 记录 ---
            writer.add_scalar("Train/01_Episode_Reward", episode_return, i)
            writer.add_scalar("Train/02_Epsilon", agent.epsilon, i)
            writer.add_scalar("Train/03_Avg_Loss", avg_loss, i)
            writer.add_scalar("Train/04_Rolling_Success_Rate", rolling_success_rate, i)
            writer.add_scalar("Train/05_Episode_Steps", episode_steps, i)

            # --- 屏幕打印日志 (每一回合打印一次) ---
            log_msg = (
                f"Ep: {i+1}/{cfg.episodes} | "
                f"Rew: {episode_return:.1f} | "
                f"Steps: {episode_steps} | "
                f"SucRate({len(success_sliding_window)}): {rolling_success_rate:.1%} | "
                f"Loss: {avg_loss:.4f} | "
                f"Eps: {agent.epsilon:.2f}"
            )
            log_tqdm(log_msg)
            
            # 更新进度条完成一回合
            pbar.update(1)

    # 6. 保存原始数据与训练指标
    raw_data_path = os.path.join(log_dir, "raw_episode_returns.csv")
    with open(raw_data_path, "w", newline="") as f:
        writer_csv = csv.writer(f)
        writer_csv.writerow(["episode", "return"])
        for idx, val in enumerate(return_list, start=1):
            writer_csv.writerow([idx, val])
    log_line(f"原始回报数据已保存至 (CSV): {raw_data_path}")

    metrics = compute_training_metrics(
        return_list,
        None,
        success_threshold=SUCCESS_THRESHOLD,
        cumulative_target=CUMULATIVE_TARGET,
        custom_bins=REWARD_BIN_RULES,
        success_reward_cutoff=SUCCESS_REWARD_CUTOFF,
        success_target_count=SUCCESS_TARGET_COUNT,
        reward_norm_range=REWARD_NORM_RANGE,
    )
    save_metrics(log_dir, metrics)

    # 7. 关闭环境与绘图
    env.close()
    writer.close()
    
    plt.figure(figsize=(10, 6))
    plt.plot(return_list, color='#003f5c', label='Episode Return')
    # 绘制滑动平均曲线
    if len(return_list) >= SUCCESS_WINDOW_SIZE:
        means = np.convolve(return_list, np.ones(SUCCESS_WINDOW_SIZE)/SUCCESS_WINDOW_SIZE, mode='valid')
        plt.plot(np.arange(SUCCESS_WINDOW_SIZE-1, len(return_list)), means, color='#ffa600', label='Rolling Average')
    
    plt.title(f'DQN Training Result on {ENV_NAME}')
    plt.xlabel('Episodes')
    plt.ylabel('Return')
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(log_dir, 'dqn_training_result.png'))
    log_line(f"训练结束，结果已保存至 {log_dir}\n")
    log_line(f"清除仿真残留进程： killall -9 gzserver gzclient rosmaster rosout robot_state_publisher rviz python python3 \n")

if __name__ == '__main__':
    train_dqn()