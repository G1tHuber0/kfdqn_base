...
def train_ac():
    cfg = Config(algo='AC', env_name=ENV_NAME) 
    try:
    ...
    log_dir = os.path.join("results_GoalReachROS/AC", f"AC_{curr_time}")
    ...
    with tqdm(total=cfg.episodes, desc=f"AC {ENV_NAME}", unit="ep", dynamic_ncols=True, colour='cyan', position=tqdm_position, leave=True) as pbar:      
    ...
    plt.plot(return_list, color='#1b9e77', label="Episode Return")
    ...
    plt.title(f'Actor-Critic Training Result on {ENV_NAME}')
    plt.xlabel('Episodes')
    plt.ylabel('Return')
    plt.legend()
