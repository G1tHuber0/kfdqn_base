...
def train_kfdqn():
    ...
    log_dir = os.path.join("results_GoalReachROS/KFDQN", f"KFDQN_{curr_time}")
    ...
    with tqdm(total=cfg.episodes, desc="KFDQN", unit="ep", dynamic_ncols=True, colour='red', position=tqdm_position, leave=True) as pbar:
        for ep in range(cfg.episodes):
            ...
            writer.add_scalar("Train/04_Rolling_Success_Rate", rolling_success_rate, ep)
            writer.add_scalar("Train/05_Episode_Steps", episode_steps, ep)
            ...
            log_msg = (
                f"Ep:{ep+1}/{cfg.episodes} | "
                f"Rew:{ep_return:.1f} | "
                f"Steps:{episode_steps} | "
                f"SucRate({len(success_sliding_window)}): {rolling_success_rate:.1%} | "
                f"Q_Loss:{avg_q_loss:.4f} | "
                f"Fuzzy_Loss:{avg_fuzzy_loss:.4f} | "
                f"Eps:{agent.epsilon:.2f} | m:{agent.m:.2f}"
            )
            log_tqdm(log_msg)
            pbar.update(1)
    ...
    plt.plot(return_list,  color="#005d9b", label="Episode Return") 
    ...
    plt.title(f'KFDQN Training Result on {ENV_NAME}')
    plt.xlabel('Episodes')
    plt.ylabel('Return')
    plt.legend()
