...
def train_reinforce():
    cfg = Config(algo='Reinforce', env_name=ENV_NAME)
    try:
    ...
    log_dir = os.path.join("results_GoalReachROS/Reinforce", f"Reinforce_{curr_time}")
    ...
    with tqdm(total=cfg.episodes, desc=f"Reinforce {ENV_NAME}", unit="ep", dynamic_ncols=True, colour='blue', position=tqdm_position, leave=True) as pbar:      
    ...
    plt.plot(return_list, color="#034fc2", label="Episode Return")
    ...
    plt.title(f'REINFORCE Training Result on {ENV_NAME}')
    plt.xlabel('Episodes')
    plt.ylabel('Return')
    plt.legend()
