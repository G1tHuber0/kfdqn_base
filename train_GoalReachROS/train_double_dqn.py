...
def train_double_dqn():
    cfg = Config(algo='Double', env_name=ENV_NAME)
    try:
    ...
    log_dir = os.path.join("results_GoalReachROS/DoubleDQN", f"DoubleDQN_{curr_time}")
    ...
    with tqdm(total=cfg.episodes, desc=f"DoubleDQN {ENV_NAME}", unit="ep", dynamic_ncols=True, colour='magenta', position=tqdm_position, leave=True) as pbar:
    ...
    plt.plot(return_list, color="#004ca3", label="Episode Return")
    ...
    plt.title(f'Double DQN Training Result on {ENV_NAME}')
    plt.xlabel('Episodes')
    plt.ylabel('Return')
    plt.legend()
